...
## Parte 4 — Demonstração empírica do efeito no Copilot (A/B)

Para fechar o ciclo gerar→avaliar→iterar de forma empírica (não só conceitual), comparamos **a mesma tarefa** gerada por um agente em duas condições: **sem** e **com** a seção Testing Standards no contexto.

**Tarefa dada ao agente:** *"Gere um teste de integração para o caso de devolução de carga perigosa (VC-03) no query endpoint."*

### Condição A — SEM a seção no contexto

```typescript
// Gerado sem Testing Standards no contexto
import { handler } from '../../src/functions/query/handler';

test('hazardous cargo return', async () => {
  const result = await handler({
    body: JSON.stringify({ question: 'Posso devolver carga perigosa?' }),
  });
  expect(result).toBeDefined();
...
Falhas: bate no serviço real (sem msw); `toBeDefined()` + só `status`; **não verifica o guardrail** (não checa a negativa nem o "4500"); sem `source_document`; nome fraco; sem AAA. **Crítico:** este teste fica verde mesmo que o modelo responda "sim, carga perigosa pode ser devolvida em 7 dias" — o falso verde no risco mais grave.

### Condição B — COM a seção no contexto

```typescript
// tests/integration/query-handler.hazardous.test.ts
import { describe, it, expect, beforeAll, afterEach, afterAll } from 'vitest';
import { setupServer } from 'msw/node';
import { http, HttpResponse } from 'msw';
import { handler } from '../../src/functions/query/handler';
...
describe('queryHandler', () => {
  ...
  it('should refuse return instructions for hazardous cargo and point to Gestão de Riscos', async () => {
    // Arrange
    const question = goldenQueries.devolverCargaPerigosa;
    server.use(
      http.post('https://*.search.windows.net/*', () =>
    ...
    expect(payload.answer).not.toMatch(/pode ser devolvid|7 dias/i); // assertion de ausência (regra 4)
  });
});
```

### Diferença medida entre A e B

| Dimensão | Sem a seção (A) | Com a seção (B) |
|----------|------------------|-----------------|
| Nomenclatura descritiva | ✗ `'hazardous cargo return'` | ✓ `should refuse ... and point to Gestão de Riscos` |
| AAA explícito | ✗ | ✓ |
| Mock na borda (msw) | ✗ (rede real) | ✓ |
| Dados de fixture | ✗ (string inline) | ✓ (`goldenQueries`, `polChunks`) |
| Assertion de contrato (`source_document`) | ✗ | ✓ |
| **Assertion de guardrail (`not.toMatch`)** | ✗ | ✓ |
| Rastreabilidade ao VC | ✗ | ✓ (`// VC-03`) |
| Pega o falso verde da carga perigosa? | **Não** | **Sim** |

**Conclusão:** a seção mudou 7 de 7 dimensões e, decisivamente, fez o agente gerar a **assertion de ausência** que captura a falha mais perigosa do domínio. Sem a seção, o teste daria verde para uma resposta proibida.

### Como reproduzir com o Copilot real

1. Abra o repositório **sem** a seção Testing Standards no `AGENTS.md`; peça ao Copilot a tarefa acima; salve a saída (condição A).
2. Cole a seção da Parte 1 no `AGENTS.md`; repita a mesma tarefa; salve a saída (condição B).
3. Compare A e B com os 6 critérios da Parte 3 — em especial o critério 6 (assertion de ausência). O delta é a evidência objetiva de que a seção melhora o output.
